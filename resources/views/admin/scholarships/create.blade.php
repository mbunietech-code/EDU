<x-layouts.admin title="Add Scholarship" header="Add Scholarship">

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('admin.scholarships.store') }}" enctype="multipart/form-data" class="mbui-card p-6 space-y-5">
            @csrf
            <div>
                <x-input-label for="title" value="Title" />
                <x-text-input id="title" class="mbui-input mt-1" type="text" name="title" :value="old('title')" required />
                <x-input-error :messages="$errors->get('title')" class="mt-2" />
            </div>
            <div>
                <x-input-label for="slug" value="Slug" />
                <x-text-input id="slug" class="mbui-input mt-1" type="text" name="slug" :value="old('slug')" placeholder="scholarship-name" required />
                <x-input-error :messages="$errors->get('slug')" class="mt-2" />
            </div>
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    <x-input-label for="country" value="Country" />
                    <x-text-input id="country" class="mbui-input mt-1" type="text" name="country" :value="old('country')" placeholder="e.g. United Kingdom" />
                    <x-input-error :messages="$errors->get('country')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="deadline" value="Application deadline" />
                    <x-text-input id="deadline" class="mbui-input mt-1" type="date" name="deadline" :value="old('deadline')" />
                    <x-input-error :messages="$errors->get('deadline')" class="mt-2" />
                </div>
            </div>
            <div>
                <x-input-label for="description" value="Description" />
                <textarea id="description" name="description" rows="5" class="mbui-input mt-1">{{ old('description') }}</textarea>
                <x-input-error :messages="$errors->get('description')" class="mt-2" />
            </div>
            <div>
                <x-input-label for="apply_url" value="Apply link" />
                <x-text-input id="apply_url" class="mbui-input mt-1" type="url" name="apply_url" :value="old('apply_url')" placeholder="https://..." />
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-5">
                ...
                <x-mbui.button type="submit">Create scholarship</x-mbui.button>
            </div>
        </form>
    </div>
