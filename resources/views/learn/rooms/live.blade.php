{{-- Live classroom (spec §12). All behaviour lives in resources/js/learning/classroom.js. --}}
<x-layouts.classroom :title="$room->title">
    <div class="flex min-h-0 flex-1 flex-col" x-data="learnClassroom(@js($config))" x-cloak
        ...
        {{-- Connection lost banner (feed polling backs off exponentially) --}}
        <div x-show="showConnectionBanner" x-transition.opacity role="status" aria-live="polite"
            ...
            <span x-text="online ? 'Connection lost — retrying…' : 'You are offline — we will reconnect when your connection returns.'"></span>
        </div>
