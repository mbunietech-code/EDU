<x-layouts.user title="Research & Consultancy" header="Research & Consultancy">

    <div class="mbui-page-header">
        <div>
            <h1 class="mbui-title">Research &amp; Consultancy</h1>
            <p class="mt-1 text-sm text-gray-500">A growing library of research work and consultancy guides, reviewed before publishing.</p>
        </div>
        @if (auth()->user()?->canWriteResearch())
            <a href="{{ route('research.contributor.index') }}"
               class="inline-flex items-center justify-center rounded-lg bg-white px-4 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">My Research</a>
        @endif
    </div>
        ...
        <div class="mt-8">
            ...
            <div class="mt-3 grid gap-3 sm:grid-cols-3">
                @foreach ($continue as $p)
                    <a href="{{ route('research.show', $p->research) }}" class="mbui-card p-4 transition hover:shadow-md">
        ...
        @else
            <div class="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($categories as $category)
                    ...
                    <a href="{{ route('research.category', $category) }}" class="mbui-card group flex items-start gap-3 p-4 transition hover:shadow-md">
        ...
        <div class="mt-10">
            ...
            <div class="mt-3 space-y-2">
                @foreach ($recent as $research)
                    <a href="{{ route('research.show', $research) }}" class="mbui-card flex items-center justify-between gap-4 p-4 transition hover:shadow-md">
                        <div class="min-w-0">
