<x-layouts.user :title="$research->title" :header="'Edit — ' . $research->title">
    ...
    @if ($locked)
        <x-mbui.alert type="info" class="mt-4">
            This research is <strong>{{ $research->statusLabel() }}</strong> and is read-only right now.
        </x-mbui.alert>
    @endif
            ...
            @unless ($locked)
                <x-mbui.card class="p-5">
                    <form method="POST" action="{{ route('research.contributor.chapter.store', $research) }}" class="flex items-end gap-2">
                        ...
                        <x-mbui.button type="submit">Add</x-mbui.button>
                    </form>
                </x-mbui.card>
            @endunless
        </div>
