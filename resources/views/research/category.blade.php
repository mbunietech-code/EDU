<x-layouts.user :title="$category->name" :header="$category->name">
    ...
    <div class="mt-6 space-y-2">
        @forelse ($researches as $research)
            <a href="{{ route('research.show', $research) }}" class="mbui-card block p-5 transition hover:shadow-md">
                ...
                @endif
                <p class="mt-2 text-xs text-gray-400">by {{ $research->author->name ?? 'Contributor' }} · published {{ $research->published_at?->diffForHumans() }}</p>
            </a>
        @empty
            <x-mbui.card>
                <x-mbui.empty-state title="Nothing here yet" message="Check back soon — new work is added as it gets approved." />
            </x-mbui.card>
        @endforelse
    </div>
